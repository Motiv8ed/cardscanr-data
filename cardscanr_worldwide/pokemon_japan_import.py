"""Import official Japanese Pokémon TCG card checkpoints into worldwide staging."""
...
def _source_record(
    ...
    record_id: str, path: str, payload: dict[str, Any],
) -> str:
    raw = canonical_json(payload)
    source_id = stable_id(PROVIDER_ID, record_type, record_id, digest(raw)[:16])
    connection.execute(
        ...
        (source_id, PROVIDER_ID, run_id, snapshot_id, record_type, record_id, path, digest(raw), raw),
    )
    return source_id
...
def _prepare_releases(
    ...
    for release_id, code in connection.execute(
        ...
        existing[normalized_code(code)].append(release_id)
    grouped: dict[str, list[tuple[sqlite3.Row, dict[str, Any]]]] = defaultdict(list)
    for row, card in cards:
        if card.get("set_code"):
            grouped[normalized_code(card["set_code"])].append((row, card))
    result: dict[str, str] = {}
    for key, members in grouped.items():
        if len(existing.get(key, [])) == 1:
            result[key] = existing[key][0]
            counters["matched_set_releases"] += 1
            continue
        code = str(members[0][1]["set_code"])
        names = [name for _, card in members for name in card.get("set_names") or []]
        local_name = Counter(names).most_common(1)[0][0] if names else code
        totals = [int(card["printed_total"]) for _, card in members if str(card.get("printed_total") or "").isdigit()]
        official_count = max(totals) if totals else len(members)
        payload = {"set_code": code, "local_name": local_name, "official_count": official_count,
                   "official_card_ids": [row["provider_record_id"] for row, _ in members]}
        source_id = _source_record(connection, run_id, snapshot_id, "set", code, f"checkpoint:sets/{code}", payload)
        series_id = stable_id(PROVIDER_ID, "series", "official")
        connection.execute(
            """insert into series values (?, ?, 'official', ?, 'Official Japanese product chronology', ?)
            on conflict(id) do update set source_record_id=excluded.source_record_id""",
            (series_id, PROVIDER_ID, source_id, canonical_json({LANGUAGE: "公式商品年表"})),
        )
        ...
        connection.execute(
            ...
            (set_id, series_id, PROVIDER_ID, code, source_id, local_name,
             canonical_json({LANGUAGE: local_name}), official_count),
        )
        connection.execute(
            ...
            (release_id, set_id, LANGUAGE, REGION, local_name, code, official_count, source_id),
        )
        _mapping(connection, "set", code, "card_set", set_id, source_id, "direct_official_record")
        result[key] = release_id
        counters["created_set_releases"] += 1
    return result


def _printing_matches(connection: sqlite3.Connection, release_id: str, number: str | None) -> list[str]:
    target = normalized_number(number)
    return [printing_id for printing_id, collector in connection.execute(
        "select id,collector_number from card_printing where set_release_id=?", (release_id,),
    ) if normalized_number(collector) == target]


def _variant(connection: sqlite3.Connection, printing_id: str) -> str:
    rows = connection.execute(
        """select id,variant_key from card_variant where card_printing_id=?
        order by case when variant_key in ('depiction-unspecified','unspecified') then 0 else 1 end,id""",
        (printing_id,),
    ).fetchall()
    for variant_id, key in rows:
        if key in {"depiction-unspecified", "unspecified"}:
            return variant_id
    if len(rows) == 1:
        return rows[0][0]
...
def _import_card(
    connection: sqlite3.Connection, row: sqlite3.Row, card: dict[str, Any], run_id: str,
    snapshot_id: str, release_id: str, counters: Counter[str],
) -> None:
    record_id = row["provider_record_id"]
    payload = {"raw_sha256": row["raw_sha256"], "source_url": row["source_url"], "parsed": card}
    source_id = _source_record(connection, run_id, snapshot_id, "card", record_id,
                               f"checkpoint:cards/{record_id}", payload)
    matches = _printing_matches(connection, release_id, card.get("collector_number"))
    if len(matches) == 1:
        printing_id = matches[0]
        variant_id = _variant(connection, printing_id)
        connection.execute(
            """update card_printing set rarity=coalesce(?,rarity),illustrator=coalesce(?,illustrator),
            stage=coalesce(?,stage),hp=coalesce(?,hp),verification_status='verified' where id=?""",
            (card.get("rarity"), card.get("illustrator"), card.get("stage"), card.get("hp"), printing_id),
        )
        counters["matched_printings"] += 1
    elif not matches:
        design_id = stable_id(PROVIDER_ID, "design", record_id)
        printing_id = stable_id(PROVIDER_ID, "printing", record_id, LANGUAGE, REGION)
        variant_id = stable_id(printing_id, "official-depiction-unspecified")
        connection.execute(
            """insert into card_design values (?, 'other', ?, ?, ?)
            on conflict(id) do update set canonical_name=excluded.canonical_name""",
            (design_id, card.get("local_name"), canonical_json(card.get("national_pokedex_numbers") or []),
             stable_id(PROVIDER_ID, record_id)),
        )
        connection.execute(
            ...
            (printing_id, design_id, release_id, source_id, card.get("collector_number") or record_id,
             record_id, card.get("rarity"), card.get("illustrator"), card.get("stage"), card.get("hp"),
             canonical_json(card.get("types") or []), canonical_json(card.get("retreat_cost") or []),
             canonical_json(card.get("weaknesses") or []), canonical_json(card.get("resistances") or []),
             canonical_json(card)),
        )
        connection.execute(
            "insert or replace into card_variant values (?, ?, 'official-depiction-unspecified', null, null, null, null, 0, ?, 'unknown')",
            (variant_id, printing_id, canonical_json({"official_source": True})),
        )
        counters["created_printings"] += 1
    else:
        unresolved_id = stable_id(PROVIDER_ID, "ambiguous-printing", record_id)
        connection.execute(
            "insert or replace into unresolved_item values (?, 'source_card', ?, ?, ?, 'ambiguous_printing_match', ?, ?, 'needs_review', 0)",
            (unresolved_id, record_id, LANGUAGE, REGION,
             "Official Japanese card matches multiple printings for the same set and collector number",
             canonical_json({"printing_ids": matches, "parsed": card})),
        )
    ...
    connection.execute(
        ...
        (printing_id, LANGUAGE, card.get("local_name") or record_id, card.get("description")),
    )
    ...
    for ordinal, attack in enumerate(card.get("attacks") or []):
        if attack.get("name"):
            connection.execute("insert into attack values (?, ?, ?, ?, ?, ?, ?)",
                               (printing_id, ordinal, LANGUAGE, attack["name"], canonical_json(attack.get("cost") or []),
                                attack.get("damage"), attack.get("effect")))
    connection.execute("delete from ability where card_printing_id=? and language_code=?", (printing_id, LANGUAGE))
    for ordinal, ability in enumerate(card.get("abilities") or []):
        if ability.get("name"):
            connection.execute("insert into ability values (?, ?, ?, ?, ?, ?)",
                               (printing_id, ordinal, LANGUAGE, ability["name"], "ability", ability.get("effect")))
    ...
    _mapping(connection, "card", record_id, "card_printing", printing_id, source_id, "set_collector_exact")
    _mapping(connection, "card", record_id, "card_variant", variant_id, source_id, "official_depiction")
    connection.execute(
...
def import_checkpoint(database: Path, checkpoint_path: Path) -> dict[str, int]:
    ...
    if checkpoint.execute("select count(*) from collector_runs where status='running'").fetchone()[0]:
        checkpoint.close()
        raise RuntimeError("Japanese card checkpoint still has a running collector")
    ...
    try:
        connection.execute(
            """insert into source_provider values (?, 'Pokémon Japan official card database', 'official',
            'https://www.pokemon-card.com/card-search/', 'metadata_only', 'Pokémon Card Game Japan',
            'https://www.pokemon-card.com/terms/', null)
            on conflict(id) do update set rights_status=excluded.rights_status""", (PROVIDER_ID,),
        )
        connection.execute("insert into import_run values (?, ?, 'running', ?, ?, '{}', '{}', ?, null, null)",
        ...
        parsed = [(row, json.loads(row["parsed_json"])) for row in checkpoint.execute(
            "select * from cards where status='parsed' order by cast(provider_record_id as integer)")]
        releases = _prepare_releases(connection, run_id, snapshot_id, parsed, counters)
        for row, card in parsed:
            key = normalized_code(card.get("set_code"))
            if key and key in releases:
        ...
        for row in checkpoint.execute("select provider_record_id,local_name,status,error from cards where status!='parsed'"):
            ...
            connection.execute(
                ...
                (unresolved_id, row["provider_record_id"], LANGUAGE, REGION,
                 "Official Japanese card identity enumerated but detail page has not yet been collected",
                 canonical_json({"local_name": row["local_name"], "status": row["status"], "error": row["error"]})),
            )
        ...
        connection.execute(
            ...
            (canonical_json(dict(counters)), canonical_json({"complete": counters["uncollected_card_details"] == 0}),
